	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!typingShit.hasFocus)
		{
			...
			if (!FlxG.keys.pressed.SHIFT)
			{
				if (FlxG.keys.pressed.W || controls.UP || FlxG.keys.pressed.S || controls.DOWN)
				{
					songMusic.pause();
					vocals.pause();
					...
					if (FlxG.keys.pressed.W || controls.UP)
					{
			...
			else
			{
				if (FlxG.keys.justPressed.W || FlxG.keys.justPressed.S)
				{
					...
					if (FlxG.keys.justPressed.W || controls.UP)
					{
						songMusic.time -= daTime;
					}
					else
		...
		if (FlxG.keys.pressed.SHIFT)
			shiftThing = 4;
		if (FlxG.keys.justPressed.RIGHT || controls.RIGHT_P || FlxG.keys.justPressed.D)
			changeSection(curSection + shiftThing);
		if (FlxG.keys.justPressed.LEFT || controls.LEFT_P || FlxG.keys.justPressed.A)
			changeSection(curSection - shiftThing);
